package com.zhujuming.vip.common.utils;

import com.zhujuming.vip.common.exception.ExpenseException;
import com.zhujuming.vip.common.exception.GlobalException;
import com.zhujuming.vip.common.exception.InvalidDataException;
...
public class ValidationUtil {
    ...
    * */
     ...
     *  exception
     */
    private static <T extends ExpenseException> void checkResolveException(T ee) {
        Throwable t = ee.getE();
        if (t instanceof GlobalException) {
            ee.setCode(((GlobalException)t).getCode());
            ee.setMessage(StringUtils.isBlank(ee.getMessage()) ? "operate error" : ee.getMessage());
        } else {
            ee.setCode(500);
            ee.setMessage("operate error");
        }
    }
     ...
     * 获取异常信息
     */
    private static <T extends ExpenseException> void getExceptionMessage(Throwable ex, T ee) {
        Throwable e = ex;
        ...
        ee.setE(ex);
        ee.setMessage(sb.toString());
    }

    public static <T extends ExpenseException> void getException(Throwable ex, T ee) {
        getExceptionMessage(ex, ee);
        ValidationUtil.checkResolveException(ee);
    }
}