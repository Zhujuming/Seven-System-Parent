...
import com.zhujuming.vip.controller.TokenBuilder;
import com.zhujuming.vip.handler.GrantedAuthorityImpl;
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(userInfo)) {
                TUsersEntity usersEntity = JSON.parseObject(userInfo, TUsersEntity.class);
                ...
                if ((expiration - now) < 10 * 60 * 1000) {
                    token = TokenBuilder.builderToken(usersEntity);
                }


                response.setHeader("Access-token", token);
                UsernamePasswordAuthenticationToken upat = new UsernamePasswordAuthenticationToken(usersEntity, null, authorities);
